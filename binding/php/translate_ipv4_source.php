<?php
/**
 * 批量翻译 ip2region IPv4 源数据（OpenAI 兼容接口）
 *
 * 用法示例：
 * php binding/php/translate_ipv4_source.php \
 *   --input=data/ipv4_source.txt \
 *   --output=data/ipv4_source_zh.txt \
 *   --model=gpt-4o-mini
 *
 * 可选参数：
 * --api-key=xxx                    不传则读取环境变量 OPENAI_API_KEY
 * --base-url=https://api.openai.com/v1/chat/completions
 * --batch-size=80                  每批翻译多少个唯一词条
 * --sleep-ms=300                   每批请求后暂停毫秒数
 */
...
function printHelp(string $script): void
{
    ...
    echo "options:\n";
    echo "  --input=PATH           输入文件，默认 data/ipv4_source.txt\n";
    echo "  --output=PATH          输出文件，默认 data/ipv4_source_zh.txt\n";
    echo "  --api-key=KEY          API Key（可用 OPENAI_API_KEY 环境变量）\n";
    echo "  --base-url=URL         OpenAI 兼容接口地址\n";
    echo "  --model=NAME           模型名，默认 gpt-4o-mini\n";
    echo "  --batch-size=N         每批唯一词条数，默认 80\n";
    echo "  --sleep-ms=N           每批请求后暂停毫秒，默认 300\n";
...
function parseArgs(array $argv): array
{
    $args = [
        ...
        'output' => 'data/ipv4_source_zh.txt',
        'api-key' => '<key>',
        'base-url' => '<baseApi>/v1/chat/completions',
        'model' => 'qwen-mt-plus',
        'batch-size' => 200,
        'sleep-ms' => 1000,
        'flush-every' => 20,
